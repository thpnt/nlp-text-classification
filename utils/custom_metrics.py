...
# Custom Loss
weights = pd.Series([0.334298, 0.665702]) # Useful to load model for faster notebook iterations
...
class PrecisionMulticlass(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        for i in range(self.n_class):
            ...
            false_positive = tf.reduce_sum(tf.cast(
                tf.logical_and(tf.equal(y_true[:, i], 0), tf.equal(y_pred[:, i], 1)),
                tf.int64)
                )
